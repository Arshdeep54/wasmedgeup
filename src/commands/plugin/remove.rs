...
impl CommandExecutor for PluginRemoveArgs {
    ...
    async fn execute(self, _ctx: CommandContext) -> Result<()> {
        ...
        let mut by_name: std::collections::BTreeMap<String, Vec<std::path::PathBuf>> =
            std::collections::BTreeMap::new();

        let mut searched_dirs: Vec<std::path::PathBuf> = Vec::new();
        ...
        let mut removed_targets: std::collections::HashSet<std::path::PathBuf> =
...
pub fn extract_plugin_name(path: &std::path::Path) -> Option<String> {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        let core = &fname["libwasmedgePlugin".len()..fname.len() - ".so".len()];
        Some(core.to_string())
    }
    #[cfg(target_os = "macos")]
    {
        ...
        let core = &fname["libwasmedgePlugin".len()..fname.len() - ".dylib".len()];
        Some(core.to_string())
    }
    #[cfg(target_os = "windows")]
    {
        ...
        let core = &fname["wasmedgePlugin".len()..fname.len() - ".dll".len()];
        Some(core.to_string())
    }
}
